...
/*   Updated: 2022/01/02 03:43:08 by kpucylo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	checks(char *s, va_list args, size_t i, int *len)
{
	if (s[i] == 'c')
		...
		print_unsdigit(va_arg(args, unsigned int), len);
	else if (s[i] == 'x')
		print_lhex(va_arg(args, unsigned int), len);
	else if (s[i] == 'X')
		print_uphex(va_arg(args, unsigned int), len);
	else if (s[i] == '%')
		print_char('%', len);
}
